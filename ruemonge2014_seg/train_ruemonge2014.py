...
os.system('cp train_ruemonge2014.py %s' % (LOG_DIR)) # bkp of train procedure
...
trainlist = np.tile(trainlist,100)
...
def train_one_epoch(sess, ops, next_train_element, train_writer):
    ...
    while True:
        try:
            ...
            for b in range(bsize):
                correct = np.sum(pred_val[b] == batch_label[b])
                total_correct += correct
                total_seen += pred_val.shape[1]
            loss_sum += loss_val
            if (batch_idx+1)%50 == 0:
...
def eval_one_epoch(sess, ops, next_test_element, test_writer):
    ...
    while True:
        try:
            ...
            for b in range(bsize):
                correct = np.sum(pred_label[b]==batch_label[b])
                total_correct += correct
                total_seen += pred_label.shape[1]

                for l in range(NUM_CLASSES):
                    total_seen_class[l] += np.sum(batch_label[b]==l)
                    total_correct_class[l] += (np.sum((pred_label[b]==l) \
                                                    &(batch_label[b]==l)))
                    total_union_class[l] += (np.sum((pred_label[b]==l) \
                                                   |(batch_label[b]==l)))
